// This determines how much higher above the usual position the enemy Pokémon
// is during battle. Species that float or fly have nonzero values.
const u8 gEnemyMonElevation[NUM_SPECIES] =
{
    [SPECIES_BUTTERFREE] = 13,
    [SPECIES_BEEDRILL] = 10,
    [SPECIES_PIDGEOT] = 8,
    [SPECIES_FEAROW] = 6,
    [SPECIES_ZUBAT] = 12,
    [SPECIES_GOLBAT] = 12,
    [SPECIES_VENOMOTH] = 16,
    [SPECIES_TENTACOOL] = 6,
    [SPECIES_TENTACRUEL] = 8,
    [SPECIES_MAGNEMITE] = 16,
    [SPECIES_MAGNETON] = 12,
    [SPECIES_CLOYSTER] = 7,
    [SPECIES_GASTLY] = 13,
    [SPECIES_HAUNTER] = 14,
    [SPECIES_KOFFING] = 14,
    [SPECIES_WEEZING] = 3,
    [SPECIES_HORSEA] = 14,
    [SPECIES_SEADRA] = 8,
    [SPECIES_GOLDEEN] = 13,
    [SPECIES_SEAKING] = 8,
    [SPECIES_MAGIKARP] = 7,
    [SPECIES_KABUTO] = 14,
    [SPECIES_AERODACTYL] = 9,
    [SPECIES_ZAPDOS] = 11,
    [SPECIES_MEW] = 14,
    [SPECIES_CROBAT] = 10,
    [SPECIES_CHINCHOU] = 14,
    [SPECIES_LANTURN] = 12,
    [SPECIES_TOGETIC] = 8,
    [SPECIES_HOPPIP] = 16,
    [SPECIES_SKIPLOOM] = 16,
    [SPECIES_JUMPLUFF] = 13,
    [SPECIES_YANMA] = 16,
    [SPECIES_MURKROW] = 16,
    [SPECIES_MISDREAVUS] = 16,
    [SPECIES_UNOWN] = 13,
    [SPECIES_GLIGAR] = 8,
    [SPECIES_QWILFISH] = 6,
    [SPECIES_REMORAID] = 11,
    [SPECIES_MANTINE] = 16,
    [SPECIES_KINGDRA] = 4,
    [SPECIES_PORYGON2] = 14,
    [SPECIES_LUGIA] = 6,
    [SPECIES_HO_OH] = 6,
    [SPECIES_CELEBI] = 15,
    [SPECIES_BEAUTIFLY] = 13,
    [SPECIES_DUSTOX] = 12,
    [SPECIES_NINJASK] = 15,
    [SPECIES_SHEDINJA] = 11,
    [SPECIES_TAILLOW] = 13,
    [SPECIES_SWELLOW] = 6,
    [SPECIES_WINGULL] = 15,
    [SPECIES_PELIPPER] = 8,
    [SPECIES_MASQUERAIN] = 10,
    [SPECIES_WAILORD] = 6,
    [SPECIES_CLAYDOL] = 8,
    [SPECIES_FLYGON] = 4,
    [SPECIES_LUVDISC] = 9,
    [SPECIES_VOLBEAT] = 9,
    [SPECIES_ILLUMISE] = 9,
    [SPECIES_GLALIE] = 12,
    [SPECIES_LUNATONE] = 8,
    [SPECIES_SOLROCK] = 3,
    [SPECIES_BARBOACH] = 6,
    [SPECIES_WHISCASH] = 6,
    [SPECIES_FEEBAS] = 8,
    [SPECIES_DUSKULL] = 13,
    [SPECIES_SHUPPET] = 11,
    [SPECIES_CASTFORM] = 14,
    [SPECIES_CASTFORM_SUNNY] = 11,
    [SPECIES_CASTFORM_RAINY] = 10,
    [SPECIES_CASTFORM_SNOWY] = 8,
    [SPECIES_RELICANTH] = 4,
    [SPECIES_ANORITH] = 5,
    [SPECIES_BELDUM] = 10,
    [SPECIES_METANG] = 4,
    [SPECIES_RAYQUAZA] = 6,
    [SPECIES_LATIAS] = 12,
    [SPECIES_LATIOS] = 6,
    [SPECIES_JIRACHI] = 16,
    [SPECIES_CHIMECHO] = 16,
    // Gen 4
    [SPECIES_BURMY] = 12,
    [SPECIES_BURMY_SANDY_CLOAK] = 10,
    [SPECIES_BURMY_TRASH_CLOAK] = 6,
    [SPECIES_WORMADAM] = 8,
    [SPECIES_WORMADAM_SANDY_CLOAK] = 8,
    [SPECIES_WORMADAM_TRASH_CLOAK] = 8,
    [SPECIES_MOTHIM] = 13,
    [SPECIES_COMBEE] = 16,
    [SPECIES_VESPIQUEN] = 10,
    [SPECIES_DRIFLOON] = 8,
    [SPECIES_DRIFBLIM] = 7,
    [SPECIES_MISMAGIUS] = 8,
    [SPECIES_BRONZOR] = 10,
    [SPECIES_BRONZONG] = 8,
    [SPECIES_CARNIVINE] = 11,
    [SPECIES_FINNEON] = 10,
    [SPECIES_LUMINEON] = 10,
    [SPECIES_MANTYKE] = 14,
    [SPECIES_MAGNEZONE] = 6,
    [SPECIES_TOGEKISS] = 13,
    [SPECIES_YANMEGA] = 13,
    [SPECIES_GLISCOR] = 9,
    [SPECIES_PORYGON_Z] = 9,
    [SPECIES_PROBOPASS] = 5,
    [SPECIES_DUSKNOIR] = 7,
    [SPECIES_ROTOM] = 10,
    [SPECIES_UXIE] = 12,
    [SPECIES_MESPRIT] = 10,
    [SPECIES_AZELF] = 8,
    [SPECIES_PHIONE] = 10,
    [SPECIES_MANAPHY] = 10,
    [SPECIES_DARKRAI] = 8,
    // Gen 5
    [SPECIES_MUNNA] = 9,
    [SPECIES_MUSHARNA] = 5,
    [SPECIES_WOOBAT] = 16,
    [SPECIES_SWOOBAT] = 9,
    [SPECIES_TYMPOLE] = 2,
    [SPECIES_SIGILYPH] = 6,
    [SPECIES_YAMASK] = 10,
    [SPECIES_ARCHEOPS] = 10,
    [SPECIES_SOLOSIS] = 14,
    [SPECIES_DUOSION] = 7,
    [SPECIES_REUNICLUS] = 8,
    [SPECIES_BASCULIN] = 8,
    [SPECIES_BASCULIN_BLUE_STRIPED] = 8,
    [SPECIES_EMOLGA] = 16,
    [SPECIES_ESCAVALIER] = 4,
    [SPECIES_FRILLISH] = 4,
    [SPECIES_JELLICENT] = 4,
    [SPECIES_FERROTHORN] = 10,
    [SPECIES_TYNAMO] = 16,
    [SPECIES_KLINK] = 11,
    [SPECIES_KLANG] = 6,
    [SPECIES_KLINKLANG] = 4,
    [SPECIES_ELGYEM] = 8,
    [SPECIES_BEHEEYEM] = 8,
    [SPECIES_CRYOGONAL] = 6,
    [SPECIES_LAMPENT] = 8,
    [SPECIES_CHANDELURE] = 8,
    [SPECIES_BRAVIARY] = 7,
    [SPECIES_HYDREIGON] = 3,
    [SPECIES_VOLCARONA] = 6,
    [SPECIES_TORNADUS] = 7,
    [SPECIES_THUNDURUS] = 7,
    [SPECIES_LANDORUS] = 7,
    [SPECIES_MELOETTA] = 4,
    // Gen 6
    [SPECIES_FLETCHINDER] = 9,
    [SPECIES_TALONFLAME] = 8,
    [SPECIES_VIVILLON] = 6,
    [SPECIES_FLABEBE] = 6,
    [SPECIES_FLOETTE] = 4,
    [SPECIES_FLOETTE_ETERNAL_FLOWER] = 4,
    [SPECIES_HONEDGE] = 4,
    [SPECIES_DOUBLADE] = 5,
    [SPECIES_AEGISLASH] = 3,
    [SPECIES_SPRITZEE] = 11,
    [SPECIES_INKAY] = 14,
    [SPECIES_SKRELP] = 4,
    [SPECIES_DRAGALGE] = 3,
    [SPECIES_CLAWITZER] = 6,
    [SPECIES_CARBINK] = 4,
    [SPECIES_KLEFKI] = 5,
    [SPECIES_PHANTUMP] = 5,
    [SPECIES_PUMPKABOO] = 8,
    [SPECIES_PUMPKABOO_SMALL] = 8,
    [SPECIES_PUMPKABOO_LARGE] = 8,
    [SPECIES_PUMPKABOO_SUPER] = 8,
    [SPECIES_NOIBAT] = 8,
    [SPECIES_YVELTAL] = 5,
    [SPECIES_DIANCIE] = 4,
    [SPECIES_HOOPA] = 13,
    // Gen 7
    [SPECIES_VIKAVOLT] = 8,
    [SPECIES_CUTIEFLY] = 10,
    [SPECIES_RIBOMBEE] = 6,
    [SPECIES_WISHIWASHI] = 14,
    [SPECIES_WISHIWASHI_SCHOOL] = 6,
    [SPECIES_COMFEY] = 8,
    [SPECIES_MINIOR] = 16,
    [SPECIES_MINIOR_CORE_RED] = 16,
    [SPECIES_BRUXISH] = 8,
    [SPECIES_TAPU_KOKO] = 9,
    [SPECIES_TAPU_LELE] = 8,
    [SPECIES_TAPU_BULU] = 5,
    [SPECIES_TAPU_FINI] = 6,
    [SPECIES_COSMOG] = 10,
    [SPECIES_COSMOEM] = 3,
    [SPECIES_LUNALA] = 6,
    [SPECIES_NIHILEGO] = 6,
    [SPECIES_KARTANA] = 8,
    [SPECIES_NECROZMA] = 6,
    [SPECIES_POIPOLE] = 9,
    [SPECIES_NAGANADEL] = 7,
    // Gen 8
    [SPECIES_CORVISQUIRE] = 10,
    [SPECIES_ORBEETLE] = 8,
    [SPECIES_FLAPPLE] = 9,
    [SPECIES_ARROKUDA] = 11,
    [SPECIES_SINISTEA] = 9,
    [SPECIES_POLTEAGEIST] = 11,
    [SPECIES_RUNERIGUS] = 5,
    [SPECIES_MILCERY] = 10,
    [SPECIES_FROSMOTH] = 7,
    [SPECIES_DREEPY] = 5,
    [SPECIES_DRAKLOAK] = 5,
    [SPECIES_DRAGAPULT] = 3,
    [SPECIES_ETERNATUS] = 4,
    [SPECIES_REGIELEKI] = 8,
    [SPECIES_REGIDRAGO] = 5,
    [SPECIES_OVERQWIL] = 4,
    [SPECIES_ENAMORUS] = 6,
    [SPECIES_TADBULB] = 12,
    [SPECIES_GLIMMET] = 8,
    [SPECIES_GLIMMORA] = 7,
    [SPECIES_VELUZA] = 5,
    [SPECIES_DONDOZO] = 3,
    [SPECIES_FLUTTER_MANE] = 8,
    [SPECIES_IRON_JUGULIS] = 7,
    [SPECIES_IRON_MOTH] = 6,
    [SPECIES_CHI_YU] = 8,
    [SPECIES_POLTCHAGEIST] = 16,
    [SPECIES_SINISTCHA] = 10,
    [SPECIES_PECHARUNT] = 16,
    // Alolan Forms
    [SPECIES_RAICHU_ALOLAN] = 6,
    // Galarian Forms
    [SPECIES_WEEZING_GALARIAN] = 6,
    [SPECIES_ARTICUNO_GALARIAN] = 2,
    [SPECIES_YAMASK_GALARIAN] = 10,
    [SPECIES_BRAVIARY_HISUIAN] = 5,
    [SPECIES_QWILFISH_HISUIAN] = 14,
    [SPECIES_BASCULIN_WHITE_STRIPED] = 8,
    // Other Forms
    [SPECIES_BEEDRILL_MEGA] = 5,
    [SPECIES_PIDGEOT_MEGA] = 9,
    [SPECIES_ALAKAZAM_MEGA] = 6,
    [SPECIES_PINSIR_MEGA] = 6,
    [SPECIES_GYARADOS_MEGA] = 4,
    [SPECIES_AERODACTYL_MEGA] = 6,
    [SPECIES_MEWTWO_MEGA_Y] = 6,
    [SPECIES_ALTARIA_MEGA] = 4,
    [SPECIES_GLALIE_MEGA] = 5,
    [SPECIES_METAGROSS_MEGA] = 5,
    [SPECIES_LATIAS_MEGA] = 8,
    [SPECIES_LATIOS_MEGA] = 4,
    [SPECIES_DIANCIE_MEGA] = 3,
    [SPECIES_RAYQUAZA_MEGA] = 6,
    [SPECIES_GIRATINA_ORIGIN] = 4,
    [SPECIES_ROTOM_HEAT] = 8,
    [SPECIES_ROTOM_WASH] = 8,
    [SPECIES_ROTOM_FROST] = 8,
    [SPECIES_ROTOM_FAN] = 10,
    [SPECIES_ROTOM_MOW] = 8,
    [SPECIES_THUNDURUS_THERIAN] = 7,
    [SPECIES_AEGISLASH_BLADE] = 3,
    [SPECIES_NECROZMA_DAWN_WINGS] = 6,
    [SPECIES_NECROZMA_ULTRA] = 5,
    [SPECIES_ETERNATUS_ETERNAMAX] = 11,
    [SPECIES_SKARMORY_MEGA] = 6,
    [SPECIES_CHANDELURE_MEGA] = 6,
    [SPECIES_DELPHOX_MEGA] = 5,
    [SPECIES_GRENINJA_MEGA] = 6,
    [SPECIES_DRAGALGE_MEGA] = 5,
    [SPECIES_GLIMMORA_MEGA] = 5,
    [SPECIES_DRAMPA_MEGA] = 5,
    [SPECIES_CHIMECHO_MEGA] = 5,
    [SPECIES_GARCHOMP_MEGA_Z] = 4,
    [SPECIES_ZYGARDE_MEGA] = 6,
    [SPECIES_DARKRAI_MEGA] = 6,
    [SPECIES_MEOWSTIC_MEGA] = 5,
    [SPECIES_MEOWSTIC_FEMALE_MEGA] = 5,
    [SPECIES_FLOETTE_MEGA] = 2,
    [SPECIES_CLEFABLE_MEGA] = 6,
    [SPECIES_DRAGONITE_MEGA] = 7,
    [SPECIES_EELEKTROSS_MEGA] = 4,
};
